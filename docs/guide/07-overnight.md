...
```text
/meta-mode im going to bed. migrate every caller to the new parser in a fresh worktree off <base>.
done means zero old callers, all parser fixtures pass, old api deleted.
keep a decision log. don't ask me before committing.
Use the active Harness's loop or background command until done. If it has no such command, keep the autonomous run in a persistent session. If you're truly stuck after a few hours, stop and write up why.
```
