...
2. Binary-search the cause. Form candidate hypotheses, then rule them out until one survives. Seed them with `how` over the affected subsystem and the **why** skill for regression history. Each pass takes the split that removes the most remaining possibilities, gets runtime evidence, and eliminates one side. When program state is unclear, add instrumentation or logging and read it as the code runs. Do not guess. Drive a long or stubborn hunt with the active harness's scheduler. Confirm the surviving *mechanism* with runtime evidence before the step-3 architect and interrogate fan-out.
3. Plan the fix. If it crosses a function boundary, `architect` first. Delegate implementation to a subagent using the mstack `implementer` role (default `inherit-parent`) with a specific scope. Review the diff.
