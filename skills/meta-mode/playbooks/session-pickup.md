...
1. Locate the prior trail. Use the active workspace's history adapter, a remote-run URL, or a pushed branch. Read the metadata overview and last messages first, then scan back for the decision points. Parse a long transcript in a worker and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
