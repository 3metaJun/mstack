...
3. Explore in workers with `worker type: "meta-agent"` and an explicit model per the Subagents section (the **guard-the-context-window** principle skill). Each returns file pointers, conventions, test commands, and entry points. No inlined dumps.
...
6. Run `node mstack/skills/meta-mode/scripts/check-plan.mjs <plan.md>` and fix every line it prints (the **encode-lessons-in-structure** principle skill).
7. Hand back. Post the plan path and the script's output, then stop. Execution starts on the operator's explicit go, under the execution playbook the plan names.

**Verification.** Tests alone are not sufficient verification. A PR is verified only when its unit, live, and perf boxes are all checked (the **prove-it-works** principle skill). That sentence is the verification rule. Every verification block opens with it. The live block is mandatory. Ten lanes use the configured mstack `reviewer` or `implementer` role at the PR head and drive the real surface through its control skill, per the **swarm** skill. Each lane is one box with a concrete scenario, the screenshot it saves, and its pass predicate. One lane is the **Regression lane against trunk.** It runs the same load-bearing scenario on trunk and head. If trunk does not have the feature, the lane records that fact and gates the behavior the diff adds plus the end state the user waits for instead of inventing a trunk result. The perf gate is dual-sided. Trunk and head must both produce the named metric. If trunk lacks the feature, also isolate the work the diff adds and set an absolute budget for that work plus the end-to-end state the user waits for. Do not claim a ratio between unlike scenarios. The perf block names the metric, the interleaved probe, the trunk baseline measured first, and the rule with the number that fails. A PR that changes an interaction is review-gated. The operator reviews it in chat with screenshots and a video before merge. A PR that changes no interaction writes `**Review gate.** None. <PR id> is not review-gated.` and no boxes under it.

**Runtime driver.** Pick a driver by surface. Browser, Electron, and web UIs use the active harness's browser driver. CLIs and TUIs use its terminal driver. Native mobile uses the repository's simulator driver. A PR that touches two surfaces gets lanes on both. A surface without a driver is a risk in Appendix C, and its live block still names how each lane runs.
...
The program runs `mstack/skills/meta-mode/playbooks/<execution playbook>.md`. <Who merges, and which PR ids are the operator's items that stop at merge-ready.>
...
- [ ] Read these from trunk at program start. Re-read them at every tick.
  - [ ] `git show origin/main:mstack/skills/meta-mode/playbooks/<execution playbook>.md`
  - [ ] `git show origin/main:mstack/skills/swarm/SKILL.md`
  - [ ] `git show origin/main:<control skill path>`
  - [ ] `git show origin/main:mstack/skills/meta-mode/playbooks/opening-a-pr.md`
  - [ ] `git show origin/main:mstack/skills/<each other leaf skill the program uses>`
...
Each live lane runs in its own configured environment at the PR head. Drive it with the active harness's runtime driver.
...
**Verify, live.** Tests alone are not sufficient verification. A PR is verified only when its unit, live, and perf boxes are all checked. Ten lanes use the configured mstack verification role at the PR head, per the boot recipe.
...
<Docs to read before editing. Which PRs get `mstack/skills/how/SKILL.md` and `mstack/skills/interrogate/SKILL.md`. The trail per `mstack/skills/show-me-your-work/SKILL.md`.>
````
