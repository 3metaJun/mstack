...
Create `orchestrate/<project-slug>/` in the current agent's store (path in the system prompt). Every file has exactly one writer. Owners publish facts, readers aggregate at read time. Use `bun "<meta-mode-tools>/orch/orch.ts"` for bookkeeping, written below as `orch`, while its canonical plain TSV and JSON stay readable without the CLI.
...
- Never resume a worker to check on it. A resume restarts an idle worker. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, the remote worker status, and the history adapter. Transcript mtime is not liveness.
...
- After a Harness restart: local workers may be gone while remote work continues. Re-read the standing orders and `units.tsv`, recompute the frontier, reattach remote work by PR and branch rather than worker id, respawn one sub-coordinator per track from its stored brief plus current state, drain, and resume. The dead session's store lock clears itself on the next write. `orch` replaces a lock whose holder pid is gone.
