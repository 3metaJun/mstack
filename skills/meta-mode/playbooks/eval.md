...
6. **Verify the chain from transcripts, not self-report.** Read each candidate's transcript through the active workspace history adapter. Do not read transcripts from unrelated workspaces. Look at which files each candidate actually opened. Grade chain-following from the files it really read plus the shape of the code, never from the candidate's own claims.
