...
If the target is vague ("why do we do it this way?" with no clear referent), make your best guess from conversation context (open files, recent edits, the current harness location, what was just discussed). State your interpretation briefly so the user can redirect if you're off, then proceed.
...
Before spawning investigators, list the available MCPs from the the current harness environment. Use the available-tools map when present. Otherwise inspect the `mcps/` directory the current harness exposes for enabled MCP servers.
...
After the Sources Consulted block, if the user's `why` question is a prethe current harness to actually changing this code, convert the lineage findings into a Preserve / Change / Avoid / Risk constraint set suitable for planning the change.
