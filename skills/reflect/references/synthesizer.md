...
- Existing-skill-first: propose `new skill via authoring workflow:` only when no existing skill is a real home, the pattern recurs, and the topic deserves its own skill.
...
| <new pattern, no existing skill is a real home> | <draft a new skill via the authoring workflow> | <new skill via authoring workflow: <kebab-name>> |
