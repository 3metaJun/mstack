---
name: no-comments
description: "Delegate comment review, fix accepted findings, and offer encodings for claimed constraints."
---

# No comments

Delegate a comment-review worker and act on accepted findings.

Defer to the comment-review worker's fresh perspective.
...
1. Delegate one read-only worker with the `reviewer` role through the active Harness delegation interface, scoped to comment review. When the optional `agents/comment-reviewer.md` instructions are available, pass them to the worker. Do not restate its rules. If delegation is unavailable, perform the same review directly.
